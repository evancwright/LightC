...
void execute()
{
	byte byte1Upper2 = (byte)(ir[0] & 0xC0); //0b11000000
	byte byte1Upper5 = (byte)(ir[0] & 0xF8); //0b11111000
	byte byte1Outer5 = (byte)(ir[0] & 0xC7); //0b11000111
	byte byte1Inner3 = (byte)((ir[0] & 0x38)>>3); ///0b00111000
	byte byte1Lower3 = (byte)((ir[0] & 0x07));//0b00000111
	byte byte1outer6 = (byte)((ir[0] & 0xCF));//0b11001111
	byte byte1Reg1 = (byte)((ir[0] & 0x38) >> 3); //0b00111000
	byte byte1Bits45 = (byte)((ir[0] & 0x30) >> 4); //0b00110000
	...
	byte byte2Outer6 = (byte)(ir[1] & 0xCF);//0b11001111
	byte byte2Bits45 = (byte)( (ir[1] & 0x30) >> 4); //0b00110000
	byte byte2Bits345 = (byte)((ir[1] & 0x38) >> 3); //0b00111000
	byte byte2Upper5 = (byte)((ir[1] & 0xF8)); //11111000
	byte byte4Outer = (byte)(ir[3] & 0xC7); //0b11000111
	byte byte4Bits345 = (byte)((ir[3] & 0x38) >> 3); //0b00111000
	...
	else if (byte1Upper5 == 0x80) add_r(byte1Lower3); //0b10000000
	...
	else if (ir[0] == 0xcb && byte2Outer5 == 0x46) //0b01000110
	{
	...
	//0b11000100
	else if (byte1Outer5 == 0xC4) call_cc(byte1Inner3);

	//compare
	else if (ir[0] == 0xbe) cp_hl();
	//0b10111000,0b00000110
	else if (byte1Upper5 == 0xB8 && byte1Lower3 != 0x06) cp_r(byte1Lower3);
	...
	//0b00000101
	else if (byte1Outer5 == 0x05) dec_rcode(byte1Inner3);
	...
	//0b00000100
	else if (byte1Outer5 == 0x04) inc_rcode(byte1Inner3);
	...
	//ld hl,r
	//0b01110000
	else if (byte1Upper5 == 0x70) ld_hl_r(byte1Lower3);

	else if (ir[0] == 0xDD &&
		byte2Upper5 == 0x70  //01110000
		)
	{//ld (ix+n),r or ld (ix+n),0
		int reg = ir[1] & 0x0F;
		ld_ix_r((byte)reg);
	}
	else if (byte1Outer5 == 0x06) //0b00000110
	{//ld r,n
		ld_r_n(byte1Inner3);
	} 
	//0b01000000, 0b00000110
	else if (byte1Upper2 == 0x40 && byte1Lower3 != 0x06)
	{
		byte r1 = byte1Reg1;
		byte r2 = byte1Lower3;
		ld_r_r(r1, r2);
	...
	//01000011
	else if (ir[0] == 0xed && byte2Outer6 == 0x43) ld_mm_rr(byte2Bits45);
	else if (opcode == 0x22) ld_mm_rr(0x02); //0b00000010
	...
	//0b01000110
	else if (byte1Outer5 == 0x46) ld_r_hl(byte1Reg1);
	//0b01000110
	else if (ir[0] == 0xDD && byte2Outer5 == 0x46) ld_r_ix(byte2Bits345);
	//0b01000110
	else if (ir[0] == 0xFD && byte2Outer5 == 0x46) ld_r_iy(byte2Bits345);
	else if (opcode == 0x3A) ld_a_mm();
	//0b01001011
	else if (ir[0] == 0xED && byte2Outer6 == 0x4b) ld_rr_mm(byte2Bits45);
	//0b00000001
	else if (byte1outer6 == 0x01) ld_rr_nn(byte1Bits45);
	...
	//0b10110000
	else if (byte1Upper5 == 0xB0) or_r(byte1Lower3);
	else if (opcode == 0xf6) or_n();
	else if (opcode == 0xb6) or_hl();
	else if (opcode == 0xddb6) or_ix();
	else if (opcode == 0xfdb6) or_iy();
	//0b11000001
	else if (byte1outer6 == 0xC1) pop_rr(byte1Bits45);
	else if (opcode == 0xdde1) pop_ix();
	else if (opcode == 0xfde1) pop_iy();
	//0b11000101
	else if (byte1outer6 == 0xC5) push_rr(byte1Bits45);
	else if (opcode == 0xdde5) push_ix();
	else if (opcode == 0xfde5) push_iy();
	//0b00000010
	else if (ir[0] == 0xCB && byte2Hi2 == 0x02) res_b_r(byte2Bits345, byte2Lower3);
	//
	else if (ir[0] == 0xDD && ir[1] == 0xCB && byte4Outer == 0x86) res_b_ix(byte4Bits345, ir[2]);
	else if (ir[0] == 0xFD && ir[1] == 0xCB && byte4Outer == 0x86) res_b_iy(byte4Bits345, ir[2]);
	//ret
	else if (opcode == 0xC9) ret();
	//set
	//0b00000010
	else if (ir[0] == 0xCB && byte2Hi2 == 0x02) set_b_r(byte2Bits345, byte2Lower3);
	//0b11000110
	else if (ir[0] == 0xDD && ir[1] == 0xCB && byte4Outer == 0xc6)
	{
